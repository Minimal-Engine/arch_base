...
BOOT_OPTS="rd.luks.uuid=${UUID_SDA2} rd.luks.uuid=${UUID_SDB1} \
rd.luks.key=${UUID_SDB1}=/crypto_keyfile.bin \
root=/dev/mapper/crypt0 rootflags=subvol=@ \
...
[Action]
Description = Mounting /boot for kernel update
When = PreTransaction
...
[Action]
Description = Unmounting /boot after kernel update
